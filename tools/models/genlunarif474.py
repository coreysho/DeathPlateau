...
beside the monster's name in orange, and a close X (831, lit 832). 522's four lines of black quill
text are replaced by two of the spell tooltips' framed black boxes, which read far better on the
stone: a row per figure, and the weakness and description. It has no background of its own: 474 laid it over the tab's stone, and so does this client
...
so the name, one line or three, sits beside the icon's middle, and centres the weakness and the
description in the lower box.
...
OPS = {'vannaka': 'Vannaka'}
PER_ROW = 4
# Monster Examine's layout (see monster_examine): name box width, the stats box's top and row height,
# where the figures start, and the height of the box for the weakness and description
NAME_W = 94
STATS_Y = 60
ROW_H = 14
VALUE_X = 112
...
# The panel's rows: (component suffix, label). Values go in val_<suffix>, left-aligned at VALUE_X.
EXAMINE_ROWS = [('level', 'Combat level'), ('hitpoints', 'Hitpoints'), ('maxhit', 'Max hit'),
                ('attack', 'Attack'), ('strength', 'Strength'), ('defence', 'Defence'),
                ('magic', 'Magic'), ('ranged', 'Ranged')]
# 474's tooltip frame (skill_magic's spell panels): two brown lines, a dark edge, and a black inside
BORDER, EDGE = '0x726451', '0x2E2B23'
LABEL, VALUE = '0xFF981F', '0xFFFFFF'


def box(f, name, x, y, w, h):
    f.com(name + '_border', [('type', 'rect'), ('x', x), ('y', y), ('width', w), ('height', h), ('colour', BORDER)])
    f.com(name + '_edge', [('type', 'rect'), ('x', x + 1), ('y', y + 1), ('width', w - 2), ('height', h - 2), ('colour', EDGE)])
    f.com(name + '_fill', [('type', 'rect'), ('x', x + 2), ('y', y + 2), ('width', w - 4), ('height', h - 4),
                           ('fill', 'yes'), ('colour', '0x000000')])


def monster_examine(cache):
    """474's icon, name and X where 522 has them. 522's four lines were black quill text straight on
    the stone, which is hard to read at a glance, so the numbers are laid out as the spell tooltips
    are instead: a framed black box, a row per figure with its name in orange and the figure in
    white, in the plain shadowed font; and under it a second box with the weakness in red and the
    Examine words. lunar_insight.rs2 fills val_<row>, name, weakness and desc, and centres the name
    beside the icon and the last box's lines inside it."""
    ...
    f = If(["// 474's Monster Examine (474 interface 522), GENERATED by LostCityServer tools/models/genlunarif474.py -",
            ...
            'type=overlay'])
    icon = c[5]
    f.com('icon', [('type', 'graphic'), ('x', icon['x']), ('y', icon['y'] - 8), ('width', icon['width']),
                   ('height', icon['height']), ('graphic', conv.sprite(icon['sprite']))])
    # beside the icon, clear of the X; the script wraps it to this width and centres it on the icon
    f.com('name', [('type', 'text'), ('x', 68), ('y', 12), ('width', NAME_W), ('height', 50), ('center', 'yes'),
                   ('font', 'b12_full'), ('shadowed', 'yes'), ('text', 'Monster name'), ('colour', LABEL)])
    y0 = STATS_Y
    box(f, 'stats', 6, y0, 178, len(EXAMINE_ROWS) * ROW_H + 10)
    for k, (key, label) in enumerate(EXAMINE_ROWS):
        y = y0 + 5 + k * ROW_H
        f.com('label_' + key, [('type', 'text'), ('x', 14), ('y', y), ('width', 96), ('height', ROW_H),
                               ('font', 'p12_full'), ('shadowed', 'yes'), ('text', label), ('colour', LABEL)])
        f.com('val_' + key, [('type', 'text'), ('x', VALUE_X), ('y', y), ('width', 176 - VALUE_X), ('height', ROW_H),
                             ('font', 'p12_full'), ('shadowed', 'yes'), ('text', '0'), ('colour', VALUE)])
    y1 = y0 + len(EXAMINE_ROWS) * ROW_H + 10 + 4
    box(f, 'about', 6, y1, 178, ABOUT_H)
    f.com('weakness', [('type', 'text'), ('x', 10), ('y', y1 + 5), ('width', 170), ('height', 12), ('center', 'yes'),
                       ('font', 'p11_full'), ('shadowed', 'yes'), ('text', ''), ('colour', '0xFF4040')])
    f.com('desc', [('type', 'text'), ('x', 10), ('y', y1 + 5), ('width', 170), ('height', ABOUT_H - 10),
                   ('center', 'yes'), ('font', 'p11_full'), ('shadowed', 'yes'), ('text', ''), ('colour', '0xDCDCDC')])
